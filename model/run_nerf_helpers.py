...
class GT_Direction_NeRF(nn.Module):
    def __init__(
        self, 
        H, 
        W, 
        ...
        super().__init__()
        self.poses = poses
        self.w2c = torch.inverse(self.poses[:, :3, :3])
        self.depth_gt = depth_gt
        self.origins = self.poses[:, :3, -1]
        self.near_plane = near_plane
        self.far_plane = far_plane
        self.predict_z_val = predict_z_val
        self.H = H
        self.W = W 
        self.focal = focal 
        
    def forward(self, x, ret_ray_batch=False):
        # without direction input, return directions only
        # x [N, 3]
        N = x.shape[0]
        M = self.poses.shape[0]
        assert x.shape[-1] == 3
        # import ipdb; ipdb.set_trace()
        vecs = x[..., None, :] - self.origins[None] # [N, M, 3]
        cam_vecs = torch.sum(vecs[..., None, :] * self.w2c[None], dim=-1)
        normalized_vecs = -1 *  cam_vecs / cam_vecs[..., -1:] * self.focal # [N, M, 3]
        # get points that are within the cam frustum
        valid_mask = torch.any(~torch.isnan(normalized_vecs), -1)
        x_coords = normalized_vecs[..., 0] + 0.5 * self.W
        x_coords_mask = torch.logical_and(x_coords <= self.W, x_coords >= 0)
        y_coords = -1 * normalized_vecs[..., 1] + 0.5 * self.H
        y_coords_mask = torch.logical_and(y_coords <= self.H, y_coords >= 0)
        valid_coord_mask = torch.logical_and(x_coords_mask, y_coords_mask) # [N, M]
        valid_mask = torch.logical_and(valid_coord_mask, valid_mask)
        vec_coords = torch.tensor([1, -1]).to(x) * normalized_vecs[..., :-1] + torch.tensor([0.5*self.W, 0.5*self.H]).to(x) # [N, M, 2]
        normalized_vec_coords = vec_coords / torch.tensor([self.W, self.H]).to(x).reshape(1, 1, 2)
        normalized_vec_coords = normalized_vec_coords * 2 - 1 # [0,1] -> [-1, 1]
        normalized_vec_coords = normalized_vec_coords.unsqueeze(-2).transpose(0, 1)  # [M, N, 1, 2]
        # depth_val: [M, 1, N, 1]
        depth_val = torch.nn.functional.grid_sample(self.depth_gt.to(x).reshape(-1, 1, self.H, self.W), normalized_vec_coords)
        # import ipdb; ipdb.set_trace()
        depth_val = depth_val.reshape(M, N).transpose(0, 1)
        valid_depth_mask = depth_val > 0
        vec_dirs = normalized_vecs / self.focal
        vec_depths = (cam_vecs * vec_dirs).sum(-1) / (vec_dirs * vec_dirs).sum(-1)
        vec_depth_mask = torch.logical_and(vec_depths > self.near_plane + 0.01, vec_depths <= torch.clamp(depth_val + 0.01, max=self.far_plane + 0.01))
        # import ipdb; ipdb.set_trace()
        depth_mask = torch.logical_and(valid_depth_mask, vec_depth_mask)
        valid_mask = torch.logical_and(depth_mask, valid_mask)
        cam_rays_o, cam_rays_d = zip(*[get_rays_by_coord(self.H, self.W, self.focal, self.poses[i], vec_coords[:, i]) for i in range(self.poses.shape[0])]) # [N, M, 3]
        cam_rays_d = torch.stack(cam_rays_d, 1)
        cam_rays_o = torch.stack(cam_rays_o, 1)
        normed_cam_rays_d = F.normalize(cam_rays_d, dim=-1)
        if ret_ray_batch:
            ray_batch = torch.cat([
                cam_rays_d, 
                cam_rays_o,
                torch.ones_like(cam_rays_d)[..., :2] * torch.tensor([self.near_plane, self.far_plane]).to(device).reshape(1, 1, 2),
                normed_cam_rays_d
                ], -1)
            return ray_batch, valid_mask
        if self.predict_z_val:
            z_val = vec_depths * torch.norm(vec_dirs, p=2, dim=-1)
            normed_cam_rays_d = torch.cat([normed_cam_rays_d, z_val[..., None]], -1)
        
        return normed_cam_rays_d, valid_mask
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_cam=0, output_ch=4, skips=[4], use_viewdirs=False, predict_visibility=False):
        """ 
        """
        ...
        if self.predict_visibility:
            self.visibility_linear = [DenseLayer(3+3, W, activation="relu")] + [DenseLayer(W, W, activation="relu") for i in range(2)] + [DenseLayer(W, 1)]
            self.visibility_linear = nn.ModuleList(self.visibility_linear)
    ...
    def get_visibility(self, x, d):
        pt_dirs = torch.cat([x[..., :3], F.normalize(d[..., :3], p=2, dim=-1)], -1)
        visibility_h = self.visibility_linear[0](pt_dirs)
        visibility_h = F.relu(visibility_h)
        for i, l in enumerate(self.visibility_linear[1:-1]):
            visibility_h = self.visibility_linear[i+1](visibility_h)
            visibility_h = F.relu(visibility_h)
        visibility = self.visibility_linear[-1](visibility_h)
        visibility = F.sigmoid(visibility)
        return visibility
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            if self.predict_visibility:
                visibility = self.get_visibility(input_pts, input_views)
                outputs = torch.cat([outputs, visibility], -1)
        else:
...
def get_rays_by_coord(H, W, focal, c2w, coords):
    i, j = (coords[:,0]-W*0.5)/focal, -(coords[:,1]-H*0.5)/focal
